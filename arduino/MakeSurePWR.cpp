 ...
 */
void MakeSurePWR::setIPF(uint8_t peripheral) 
{
	setIPF_(peripheral);
	intIPRA = IPRA;
}
 ...
 * FIXME: missing all the Timers and UART to reduce consumption
 */
void MakeSurePWR::resetIPF(uint8_t peripheral) 
{
 ...
 */
void MakeSurePWR::setSensorPower(uint8_t type, uint8_t mode)
{
 ...
 * It returns nothing
 */
void MakeSurePWR::setWatchdog(uint8_t mode, uint8_t timer)
{
	if(mode==WTD_ON)
	{
...
/* switchesON() - switches on all MakeSuremote switches
 *
 * It switches on all MakeSuremote switches
 */
void	MakeSurePWR::switchesON(uint8_t option)
{
 ...
 * It returns nothing.
 */
void MakeSurePWR::sleep(uint8_t option)
{
	uint8_t retries=0;
 ...
 * 
 * MakeSuremote enters hibernate. The device is off and no variable will be stored
 * in RAM memory
 * 
 */
void MakeSurePWR::hibernate(const char* time2wake, uint8_t offset, uint8_t mode)
{
 ...
 */
uint8_t    MakeSurePWR::getBatteryLevel()
{
   float aux=0;
   uint8_t resul=0;
   pinMode(BAT_MONITOR_PW,OUTPUT);
 ...
 */
float MakeSurePWR::getBatteryVolts()
{
   float aux_volts=0;
   pinMode(BAT_MONITOR_PW,OUTPUT);
   digitalWrite(BAT_MONITOR_PW,HIGH);
 ...
 * 
 * This function checks if MakeSuremote has been reset from an hibernate mode by 
...
void	MakeSurePWR::ifHibernate()
{
	...
	if( digitalRead(RTC_INT_PIN_MON) && (Utils.readEEPROM(HIB_ADDR)==HIB_VALUE) )
	{
		...
		// MakeSuremote startup conditions may show the hibernate startup conditions 
		// when it is not true	
	...
	if( !(intFlag & HIB_INT) )
	{
		...
		// in order to set up MakeSuremote for hibernating
...
// clear all kind of Interruption signal in MakeSuremote
void MakeSurePWR::clearInt()
{
	// RTC module
