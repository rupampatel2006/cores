 ...
 * Constructor
 */
MakeSureUtils::MakeSureUtils (void)
{
}
 ...
 */
void MakeSureUtils::setLED(uint8_t led, uint8_t state)
{
	pinMode(led,OUTPUT);
	if(state==LED_ON)
	{
 ...
 */
void	MakeSureUtils::setExternalLED(uint8_t state)
{
	pinMode(LED1,OUTPUT);
	if(state==LED_ON)
	{
 ...
 */
uint8_t	MakeSureUtils::getLED(uint8_t led)
{
 ...
 */
uint8_t	MakeSureUtils::getExternalLED()
{
 ...
 */
void MakeSureUtils::blinkLEDs(uint16_t time) 
{
	setLED(LED0,LED_ON);
	setLED(LED1,LED_ON);
	delay(time);
 ...
 */
void MakeSureUtils::externalLEDBlink(uint16_t time) 
{
 ...
 */
long MakeSureUtils::map(long x, long in_min, long in_max, long out_min, long out_max)
{
 ...
 */
void MakeSureUtils::setMux(uint8_t MUX_LOW, uint8_t MUX_HIGH)
{
 ...
 * MUX_LOW = 0 & MUX_HIGH = 0 ---> AUX2 MODULE
 */
void MakeSureUtils::setMuxSocket1()
{
	pinMode(MUX_PW, OUTPUT);
	pinMode(MUX0, OUTPUT);      
	pinMode(MUX1, OUTPUT);   
 ...
 *
*/
void MakeSureUtils::setMuxSocket0()
{
//Rupam - Need to clarify the usage of Mux
  // pinMode(MUX_PW,OUTPUT);
  // pinMode(MUX_USB_XBEE,OUTPUT);
  // digitalWrite(MUX_PW,HIGH);
 ...
 */
uint8_t MakeSureUtils::readEEPROM(int address)
{
 ...
 * EEPROM has 512 Bytes of memory
 */
void MakeSureUtils::writeEEPROM(int address, uint8_t value)
{
 ...
 */
void MakeSureUtils::setID(char* moteID)
{
 ...
 */
void MakeSureUtils::setAuthKey(char* authkey)
{
...
/* readSerialID() - reads the MakeSuremote unique serial identifier
 *
 * It reads the MakeSuremote unique serial identifier
 */
 //unsigned long MakeSureUtils::readSerialID()
 //{
	// Utils.setLED(LED0,LED_OFF);
	// Utils.setLED(LED1,LED_OFF);
	
	// MakeSureOneWire OneWire(LED1);
...
// /* readTempDS1820() - reads the DS1820 temperature sensor
 ...
 // */
// float MakeSureUtils::readTempDS1820(uint8_t pin)
// {
	// PWR.setSensorPower(SENS_3V3,SENS_ON);
	// delay(10);
	
	// MakeSureOneWire OneWireTemp(pin);
	...
	// if ( MakeSureOneWire::crc8( addr, 7) != addr[7]) 
	// {
		// return -1000;
	// }
 ...
 * 
 */
void MakeSureUtils::hex2str(uint8_t* number, char* macDest)
{
 ...
 * 
 */
void MakeSureUtils::hex2str(uint8_t* number, char* macDest, uint8_t length)
{
	uint8_t aux_1=0;
	uint8_t aux_2=0;
 ...
 */
float MakeSureUtils::readTemperature()
{
	// variables
	int sensor_value = 0;
 ...
 */
uint8_t MakeSureUtils::readHumidity()
{
	// variables
	int sensor_value = 0;
	float sensor_volts = 0;
 ...
 */
uint8_t MakeSureUtils::readLight()
{
	// variables
	int sensor_value = 0;
	float sensor_volts = 0;
...
*/
uint8_t MakeSureUtils::long2array(long num, char* numb)
{
	long aux=num;
	uint8_t i=0;
 ...
 * 
 */
uint8_t MakeSureUtils::str2hex(char* str)
{
	int aux=0, aux2=0;
 ...
 * 
 */
uint8_t MakeSureUtils::str2hex(uint8_t* str)
{
	int aux=0, aux2=0;
 ...
 * 
 */
uint8_t MakeSureUtils::converter(uint8_t conv1, uint8_t conv2)
{
	uint8_t aux=0;
	uint8_t aux2=0;
	uint8_t resul=0;
 ...
 * 
 */
void MakeSureUtils::float2String (float fl, char str[], int N) 
{
 ...
 */
void MakeSureUtils::setSPISlave(uint8_t SELECTION)
{
 ...
 * 
 */
void MakeSureUtils::loadOTA(const char* filename, uint8_t version)
{  
 ...
 */
int8_t MakeSureUtils::checkNewProgram()
{
 ...
 */
void MakeSureUtils::getProgramID(char* program_ID)
{
 ...
 */
void MakeSureUtils::getID(char* moteID)
{
	// set the mote ID to EEPROM memory
	for( int i = 0 ; i<16 ; i++ )
	{		
 ...
 */
void MakeSureUtils::setProgramVersion(uint8_t version)
{
 ...
 */
uint8_t MakeSureUtils::getProgramVersion()
{
	return (eeprom_read_byte((unsigned char *) 0xE1));
}


MakeSureUtils Utils = MakeSureUtils();
